...
value SteamWrap_GenerateItems(value itemDefs, value itemAmts, value arrayLen)
{
	...
	std::vector<SteamItemDef_t> _items;
	std::vector<uint32> _amts;
	_items.resize(_len);
	_amts.resize(_len);
	...
	SteamInventory()->GenerateItems(&_result, _items.data(), _amts.data(), _len);
	return alloc_int(_result);
}
DEFINE_PRIM(SteamWrap_GenerateItems,3);

//-----------------------------------------------------------------------------------------------------------
void SteamWrap_DestroyResult(value invHandle)
{
	SteamInventoryResult_t _resultHandle = val_int(invHandle);
...
value SteamWrap_GetResultItemProperty(value handle, value itemIndex, value propertyName)
{
	...
	uint32_t _index = val_int(itemIndex);
	std::string _propertyName = val_string(propertyName);
	const char* _propertyName_c = _propertyName.c_str();
	char* buffer_val;
